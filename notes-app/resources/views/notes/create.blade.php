...
@section('meta_description', 'Create a new AI-powered note.')
...
<div x-data="noteForm()">
    ...
    <nav class="text-sm text-gray-400 dark:text-neutral-500 mb-6">
        ...
        <span class="text-gray-600 dark:text-neutral-300">Create Note</span>
    </nav>
    ...
    <div class="bg-white dark:bg-neutral-900/50 rounded-2xl shadow-sm border border-gray-100 dark:border-neutral-800/80 p-8 max-w-3xl transition-colors duration-200">
        <h1 class="text-xl font-bold text-gray-900 dark:text-neutral-100 mb-6">Create New Note</h1>
        ...
        <form @submit.prevent="submit">
            ...
            {{-- Tags --}}
            <div class="mb-7">
                <label for="tags" class="block text-sm font-medium text-gray-700 dark:text-neutral-300 mb-1">Tags <span class="text-gray-400 dark:text-neutral-500 font-normal">(optional, comma-separated)</span></label>
                <input
                    id="tags"
                    type="text"
                    x-model="tagsInput"
                ...
                />
                <p x-show="errors.tags" class="text-xs text-red-500 mt-1" x-text="errors.tags"></p>
                <p class="text-xs text-gray-400 dark:text-neutral-500 mt-1">Maximum 10 tags, 50 characters each.</p>
            </div>
            ...
            <div class="flex gap-3">
                <button
                    ...
                    class="bg-indigo-600 text-white text-sm font-medium px-6 py-2.5 rounded-lg hover:bg-indigo-700 transition disabled:opacity-50 flex items-center gap-2">
                    <span x-show="!isSubmitting">Create Note</span>
                    <span x-show="isSubmitting">Creating...</span>
                </button>
                <a href="/notes" class="text-sm text-gray-500 dark:text-neutral-300 px-4 py-2.5 rounded-lg border border-gray-200 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800 transition">
                    Cancel
                </a>
            </div>
        </form>
        ...
        async submit() {
            ...
            try {
                ...
                if (res.status === 422) {
                    ...
                    this.formError = data.message || 'Failed to create note.';
                }
            } catch (e) {
